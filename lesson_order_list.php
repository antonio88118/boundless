...
if (isset($_GET["search_text"])) {
    ...
    $sql = sprintf(
        "SELECT lesson_order_items.id AS itemID , lesson_order_items.lessonID, lesson_order.*, users.id AS userID, users.address, users.email, users.name, users.phone, lesson.price, lesson.name AS product_name, price AS amount FROM lesson_order
    JOIN users ON lesson_order.user_id = users.id
    JOIN lesson_order_items ON lesson_order_items.id = lesson_order.orderID
    LEFT JOIN lesson ON lesson.id = lesson_order_items.lessonID
    WHERE %s LIKE '%s'
    LIMIT $startItem, $perPage",
    ...
    $sqlCount = "SELECT lesson_order.* FROM lesson_order";
    $countResult = $conn->query($sqlCount);
    $totalOrder = $countResult->num_rows; //顯示共幾筆資料

    $sql = "SELECT lesson_order_items.id AS itemID , lesson_order_items.lessonID, lesson_order.*, users.id AS userID, users.address, users.email, users.name, users.phone, lesson.price, lesson.name AS product_name, price AS amount FROM lesson_order
                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="d-flex justify-content-between">
                            <h1 class="h3 mb-2 text-gray-800">課程訂單管理</h1>
                            <div>
                                <?php if (isset($_GET["search_text"])) : ?>
                                    <a class="btn btn-dark mb-2" href="lesson_order_list.php"><i class="bi bi-caret-left-fill" title="回訂單列表"></i></a><br>
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 d-flex" style="background-color: #ACACAC;">
                            <!-- Search -->
                            <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="lesson_order_list.php" method="get">
                                <div class="input-group" style="width: 400px;">
                                    <select class="form-select" name="search_type" aria-label="Default select example" style="width: 25%;">
                                        ...
                                        <option value="lesson_order_items.id">訂單編號</option>
                                        <option value="users.name">訂購者</option>
                                        <option value="lesson.name">訂購課程</option>
                                    </select>
                                    ...
                                    <?php if ($pageCount > 1) : ?>
                                        <nav aria-label="Page navigation example">
                                            <ul class="pagination d-flex justify-content-center">
                                                <li class="page-item">
                                                    <a class="page-link" href="lesson_order_list.php?page=1" aria-label="Previous">
                                                ...
                                                <li class="page-item">
                                                    <?php if ($page == 1) : ?>
                                                        ...
                                                        <?php else : ?>
                                                            <a class="page-link" href="lesson_order_list.php?page=<?= $page - 1 ?>" aria-label="Previous">
                                                            <?php endif; ?>
                                                ...
                                                <?php if ($page > $page_left + 1) : ?>
                                                    <?php for ($i = $page - $page_left; $i < $page; $i++) : ?>
                                                        <li class="page-item <?php if ($page == $i) echo "active" ?>">
                                                            <a class="page-link" href="lesson_order_list.php?page=<?= $i ?>"><?= $i ?></a>
                                                        </li>
                                                    <?php endfor; ?>
                                                <?php else : ?>
                                                ...
                                                <li class="page-item">
                                                    <?php if ($page == $pageCount) : ?>
                                                        ...
                                                        <?php else : ?>
                                                            <a class="page-link" href="lesson_order_list.php?page=<?= $page + 1 ?>" aria-label="Next">
                                                            <?php endif; ?>
                                                            <span aria-hidden="true">&raquo;</span>
                                                            </a>
                                                </li>
                                                <li class="page-item">
                                                    <a class="page-link" href="lesson_order_list.php?page=<?= $pageCount ?>" aria-label="Previous">
